...
// 比较的文件名
const file1Name = 'file-10g-1';
const file2Name = 'file-10g-2';
// 比较文件目录前缀
...
async function main() {
    ...
    const t2 = now();
    console.log('读取文件A完成---耗时:', (t2 - t1).toFixed(3) + 'ms');
    console.log('开始读取文件B---');
    await fileSplit(file2Name);
    const t3 = now()
    console.log('读取文件B完成---耗时:', (t3 - t2).toFixed(3) + 'ms')
    console.log('开始diff---');
    // 文件对列表
    const fileList = [];
    for (let i = 0; i < fileNum; i++) {
    ...
    writeProcess.on('message', msg => {
        ...
        if(isEnd && fileList.length === 0 && processCount === 0) {
          console.log('-----结果写入完成-------');
          console.log('相同行数量', sameLineCount);
          const t4 = now();
    ...
    for (let j = 0; j< processNum; j++){
        ...
        child.on('message', (msg) => {
            ...
            sameLineCount+=lineArray.length;
            // 数据交给写进程
